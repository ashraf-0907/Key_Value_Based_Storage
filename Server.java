...
public class Server {
    ...
    public static void main(String[] args) {
        Properties config = utils.Property.loadProperties("config.properties");
        String host = config.getProperty("host", "0.0.0.0"); // Bind to all available network interfaces
        int port = Integer.parseInt(config.getProperty("port", "8081"));
        int maxSize = Integer.parseInt(config.getProperty("maxSize", "4096"));

        System.out.println("yellow");
        eventLoop = new EventLoop(); // Create an instance of EventLoop
        eventLoop.start(); // Start the EventLoop main thread
        ...
        try (ServerSocket serverSocket = new ServerSocket(port, 50, InetAddress.getByName(host))) {
            ...
            while (true) {
                ...
                System.out.println("Accepted connection from client: " + clientAddress + ":" + clientPort);

                // Dispatch clientSocket to EventLoop for processing asynchronously
                eventLoop.dispatchEvent(clientSocket, maxSize,1);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
